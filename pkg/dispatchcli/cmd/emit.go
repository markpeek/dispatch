...
func NewCmdEmit(out io.Writer, errOut io.Writer) *cobra.Command {
	...
	cmd.Flags().StringVar(&emitEventSource, "source", emitEventSource, "Event source.")
	cmd.Flags().StringVar(&emitEventID, "event-id", "", "Event ID. If empty, will be autogenerated.")
	cmd.Flags().StringVar(&emitEventType, "content-type", "", "Content type of the payload. Defaults to application/json if --data is used. For --data-from-file, it will try to detect the type.")
